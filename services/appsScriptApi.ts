import { ElectionStatus, User, Election, Candidate } from '../types';
...
const callApi = async (action: string, payload: any = {}, ttlSeconds = 0) => {
  ...
  return apiCache.fetchDeduped(cacheKey, async () => {
    const response = await fetch(API_URL, {
      ...
      // ✅ MOST IMPORTANT FIX (avoid preflight / CORS issue)
...
export const api = {
  ...
  admin: {
    ...
    getElectionBundle: async (id: string) => {
      ...
      if (data?.candidates) data.candidates = data.candidates.map(processCandidate);
      return data;
    },
    ...
    uploadFile: async (file: File, category: 'profile' | 'campaign') => {
      ...
      const url = await callApi('uploadFile', {
        base64,
        fileName: file.name,
        mimeType: file.type,
        category,
      ...
      // url is string
      return category === 'profile' ? fixDriveUrl(url) : url;
    },
    ...
    addBatchToWhitelist: async (
      ...
      return pLimit(emails, 3, async (email) => {
        try {
          ...
          console.error(`Whitelist failed: ${email}`, e);
        }
        completed++;
        if (onProgress) onProgress(completed);
        return email;
  ...
  voter: {
    ...
    hasVoted: async (electionId: string, email: string) => callApi('hasVoted', { electionId, email }, 60),
  },
