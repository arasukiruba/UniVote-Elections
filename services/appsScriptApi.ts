...
// CONFIGURATION
// ============================================================================
const API_URL = 'https://script.google.com/macros/s/AKfycbxJrqxch-QOiUZJEEpC54uR_oLMi4beUi_ngQ5TWhiQVuviD1itfIVNOC-ys8j7jPBSdA/exec';
...
const fixDriveUrl = (url: string | undefined): string => {
  ...
  if (id) {
    // Use the reliable thumbnail endpoint with large size for images
    return `https://drive.google.com/thumbnail?id=${id}&sz=w1000`;
    // Alternative: return `https://lh3.googleusercontent.com/d/${id}`;
  }
...
const callApi = async (action: string, payload: any = {}, ttlSeconds = 0) => {
  ...
  return apiCache.fetchDeduped(cacheKey, async () => {
    try {
      ...
      // 3. Set Cache
      if (ttlSeconds > 0) {
        apiCache.set(cacheKey, json.data, ttlSeconds);
      }
...
export const api = {
  auth: {
    login: async (email: string, password: string): Promise<User | null> => {
      // Cache auth lookup for 5 mins to make session re-validation fast
      return callApi('login', { email, password }, 300);
    },
    registerVoter: async (email: string, password: string) => {
      apiCache.invalidate('getWhitelist'); // Invalidate whitelist cache
  ...
  master: {
    ...
    toggleStatus: async (id: string) => {
      apiCache.invalidate('getAdmins');
      callApi('toggleStatus', { id });
    },
    deleteAdmin: async (id: string) => {
      apiCache.invalidate('getAdmins');
      callApi('deleteAdmin', { id });
    },
    getLogs: async () => []
  },

  admin: {
    // Combine specific calls into bundles where possible, or cache aggressively
    getElections: async (adminId?: string) => {
      const res = await callApi('getElections', { adminId }, 5); // Reduced TTL to 5s
      return Array.isArray(res) ? res : [];
    },
    
    // NEW: Bundle call for Election Control
    ...
    resetVotes: async (electionId: string) => {
      ...
      apiCache.invalidate(`getWhitelist`); // Whitelist has 'hasVoted' flags
    ...
    deletePosition: async (id: string) => {
      // Ideally we'd know electionId to invalidate specific bundle, but brute force invalidation is safer here
      apiCache.invalidate('getElectionBundle'); 
      apiCache.invalidate('getPositions');
      callApi('deletePosition', { id });
    },
    ...
    addCandidate: async (candidate: Omit<Candidate, 'id'>) => {
      ...
      // No URL processing needed here, assumed frontend passed a clean or direct URL from uploadFile
    ...
    uploadFile: async (file: File, category: 'profile' | 'campaign') => {
      ...
      // Fix URL before returning so immediate usage works
      return fixDriveUrl(res.url);
    },

    // Optimized Batch Upload
    uploadBatch: async (files: File[], category: 'campaign', onProgress?: (completed: number) => void) => {
      let completed = 0;
      return pLimit(files, 3, async (file) => { // Limit 3 concurrent uploads
    ...
    removeFromWhitelist: async (id: string) => {
      apiCache.invalidate(`getWhitelist`);
      callApi('removeFromWhitelist', { id });
    }
  },
  ...
  voter: {
    ...
    vote: async (electionId: string, email: string, selections: Record<string, string>) => {
      ...
      // Invalidate stats cache immediately after voting
      apiCache.invalidate('getStats');
      apiCache.invalidate('getResults');
      apiCache.invalidate('getVoterElectionBundle');
      return res;
